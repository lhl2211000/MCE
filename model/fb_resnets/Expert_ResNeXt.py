...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,groups=1, base_width=64, is_last=False):
...
class ResNext(nn.Module):
    def __init__(self, block, layers, num_experts, groups=1, width_per_group=64, dropout=None,num_classes=100, use_norm=False, reduce_dimension=False, layer3_output_dim=None,layer4_output_dim=None, returns_feat=False, s=30):
        ...
        self.base_width = width_per_group#4
        ...
        self.use_dropout = True if dropout else False#self.use_dropout=False
    ...
    def _make_layer(self, block, planes, blocks, stride=1, is_last=False):
        ...
        for i in range(1, blocks):
            layers.append(block(self.next_inplanes, planes,groups=self.groups, base_width=self.base_width,is_last=(is_last and i == blocks-1)))

        return nn.Sequential(*layers)

    def _make_projector(self, raw_feature, projector_final_dim):
        projector = nn.Sequential(nn.Linear(raw_feature, raw_feature, bias=False),
                                  # NormedLinear_without_bias(layer3_output_dim, layer3_output_dim),
                                  nn.BatchNorm1d(raw_feature),
                                  nn.ReLU(inplace=True),  # first layer
                                  nn.Linear(raw_feature, raw_feature, bias=False),
                                  # NormedLinear_without_bias(layer3_output_dim, layer3_output_dim),
                                  nn.BatchNorm1d(raw_feature),
                                  nn.ReLU(inplace=True),  # second layer
                                  nn.Linear(raw_feature, projector_final_dim),
