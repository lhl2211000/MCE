...
from data_loader.imbalance_cifar import IMBALANCECIFAR10
...
class  TestAgnosticImbalanceCIFAR10DataLoader(DataLoader):
    """
    Imbalance Cifar10 Data Loader
    """
    def __init__(self, data_dir, batch_size, shuffle=True, num_workers=0, training=True, balanced=False, retain_epoch_size=True, imb_type='exp', imb_factor=0.02, test_imb_factor=0, reverse=False):
        ...
        test_dataset = datasets.CIFAR10(data_dir, train=False, download=False, transform=test_trsfm) # test set
        
        if training:
            dataset = IMBALANCECIFAR10(data_dir, train=True, download=False, transform=train_trsfm, imb_type=imb_type, imb_factor=imb_factor)
            val_dataset = test_dataset
        else:#执行
            dataset = IMBALANCECIFAR10(data_dir, train=True, download=False, transform=train_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse)
            train_dataset = IMBALANCECIFAR10(data_dir, train=True, download=False, transform= TwoCropsTransform(train_trsfm), imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse)
            val_dataset = IMBALANCECIFAR10(data_dir, train=False, download=False, transform=test_trsfm, imb_type=imb_type, imb_factor=test_imb_factor, reverse=reverse)
...
def mic_acc_cal(preds, labels):
    if isinstance(labels, tuple):#判断labels是不是tuple类型
        assert len(labels) == 3
...
def main(config):
    ...
    model = config.init_obj('arch', module_arch) #import model.model as module_arch
    #logger.info(model)

    # run training data here just for obtain indexs for head/medium/tail classes
    #import data_loader.data_loaders as module_data
    train_data_loader = getattr(module_data, config['data_loader']['type'])(#getattr()函数用于返回一个对象属性值
        config['data_loader']['args']['data_dir'],
        batch_size=128,
        shuffle=False,
        training=True,
        num_workers=0,#这里的num_workers将会被传入的参数改成0
    ...
    #######################################
    for test_distribution in test_distribution_set:    #在不同的不平衡环境中做测试

        print(test_distribution)
        data_loader = TestAgnosticImbalanceCIFAR10DataLoader(#这个data_loader的转换器列表与训练阶段的转换器列表不同
            config['data_loader']['args']['data_dir'],
            batch_size=128,
            shuffle=False,
            training=False,
        ...
        for k in range(config["epochs"]):#epochs=5, aggregation_weight=[0.3333,0.3333,0.3333], optimizer=SGD, args=命令行参数
            weight_record = test_training(train_data_loader, config, model, aggregation_weight, optimizer, args)
            if weight_record[0]<0.05 or weight_record[1]<0.05 or weight_record[2]<0.05:#任意一个专家的权重<0.05，权重学习阶段就会提前结束
                break
        ...
        weight_record_list.append(weight_record)                       #此时的专家权重已经更新
    ...
    for txt in performance_record_list:
        print(test_distribution_set[i]+'\t')#test_distribution_set[i]是不同的测试集分布的名称
        print(*txt)          
        i+=1        
    ...
    for txt1 in weight_record_list:
        ...
        i+=1
                 ...
                 #训练集, config源代码没用上 ,训练阶段保存的模型, [0.3333,0.3333,0.3333], optimizer=SGD, args=命令行参数
def test_training(train_data_loader,  config, model,  aggregation_weight, optimizer, args):
    ...
    for i, (data, _) in enumerate(tqdm(train_data_loader)):
        data[0] = data[0].to(device)#之前的TwoCropsTransform类返回了同一个图像的两个随机裁剪的版本，并且以列表形式返回
        ...
        aggregation_softmax = torch.nn.functional.softmax(aggregation_weight) # softmax for normalization,这一行会有userwarning。加上dim=1就没有问题了，这个aggregation_softmax就是专家权重的放缩版本
        ...
        softmax_aggregation_output0 = F.softmax(aggregation_output0, dim=1) #import torch.nn.functional as F
        ...
        optimizer.zero_grad()
        loss.backward()#这里是对3个专家的权重求梯度
        optimizer.step()#这里更新的是权重
        
        losses.update(ssl_loss, data[0].shape[0])
         
    aggregation_softmax = torch.nn.functional.softmax(aggregation_weight, dim=0).detach().cpu().numpy()#所以这里的权重已经不是[0.3333,0.3333,0.3333]
...
def test_validation(data_loader, model, num_classes, aggregation_weight, device, many_shot, medium_shot, few_shot):
    ...
    acc_per_class = confusion_matrix.diag()/confusion_matrix.sum(1)#2Dtensor调用diag()，将会返回该二维矩阵的对角矩阵,sum(1)是按列求和,最终得到了每个类别下的准确率
...
if __name__ == '__main__':
    ...
    args.add_argument('--epochs', default=1, type=int, help='indices of GPUs to enable (default: all)')
    config = ConfigParser.from_args(args) #上面有代码：from parse_config import ConfigParser
    main(config)
